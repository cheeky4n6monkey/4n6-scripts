...
# v2015-08-26 Added sorting/printing for second last (name) field (doesn't seem to have "1:" prepended to email addresses like the last field)
#
...
version_string = "wp8-1-contacts.py v2015-08-26"
...
for hit in contact_hits:
    ...
    contact_entries[hit] = (last_field_str, second_last_field_str, third_last_field_str)
...
# sort by second last name field
sorted_contact_keys = sorted(contact_entries, key = lambda x : (contact_entries[x][1], contact_entries[x][1])) 
...
if (options.tsvfile != None):
    ...
    tsvof.write("Offset\tLast_Field(Name)\tSecond_Last_Field(Name)\tThird_Last_Field(Phone)\n")

    for key in sorted_contact_keys:
        #tsvof.write(hex(key).rstrip("L") + "\t" + contact_entries[key][0] + "\t" + contact_entries[key][1] + "\n")
        tsvof.write(hex(key).rstrip("L") + "\t" + contact_entries[key][0] + "\t" + contact_entries[key][1] + "\t" + contact_entries[key][2] +"\n")
